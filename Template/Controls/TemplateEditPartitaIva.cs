    ...
    public partial class TemplateEditPartitaIva : UserControl, IEditControl, IEditValue<string>
    {
        ...
        public void InitJQ()
        {
            try
            {
                ...
                JQScript = jquery.GetEditPartitaIva(editValue,mask);
            }
            catch (Exception ex)
            {
                UtilityError.Write(ex);
        ...
        private void TemplateEditPartitaIva_Load(object sender, EventArgs e)
        {
            try
            {
                SetEditing(editing);
        ...
        private bool IsVerified(bool required)
        {
            try
            {
                if (editValue != null)
                {
                    ...
                    bool isPartitaIva = (UtilityValidation.IsPartitaIva(value));
                    bool verified = (required && compiled && isPartitaIva) || (!required && (!compiled || (compiled && isPartitaIva)));
                    return verified;
                }
            }
            catch (Exception ex)
